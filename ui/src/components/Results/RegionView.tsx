    ...
    <>
      ...
      <TabContext value={value}>
        ...
        <TabPanel value='1'>
          ...
          >
            {region && <ErststimmeProWkr region={region} />}
...
export function ErststimmeProWkr({ region }: RegionProps): JSX.Element {
  ...
  useEffect(() => {
    getResults(region.id).then(ds => {
      const sonstiges_2021 = ds
        .filter(d => parteien.indexOf(d.kurzbezeichnung) == -1)
        .reduce(
          (sum, current) => sum + (current.erststimmen_prozent_2021 || 0),
          0
        )
      const sonstiges_2017 = ds
        .filter(d => parteien.indexOf(d.kurzbezeichnung) == -1)
        .reduce(
          (sum, current) => sum + (current.erststimmen_prozent_2017 || 0),
          0
        )
      const newResults = ds
        .filter(d => parteien.indexOf(d.kurzbezeichnung) > -1)
  ...
  return <Bar data={barData} options={getOptions('Erststimmenanteile')} />
}

export function RegionSummaryView({ region }: RegionProps): JSX.Element {
  const [regionSummary, setRegionSummary] = useState<RegionSummary | undefined>(
    undefined
  )
  ...
  return (
    <TableContainer sx={{ width: '500px' }}>
      <TableRow>
        <TableCell>
          <strong> Gewählt im Wahlkreis </strong>{' '}
        </TableCell>
